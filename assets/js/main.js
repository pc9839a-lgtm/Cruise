(function () {
  ...
  function renderSettings() {
    ...
    setHtml(
      ...
      convertLineBreaks(
        escapeHtml(
          settings.identity_desc || '쇼핑과 옵션이 포함된 패키지 여행이 아닙니다.\n오직 크루즈 일정과 항해 루트를 투명하게 비교하고 선택하는\n자유여행 중심 안내 플랫폼입니다.'
        )
      )
    );
  ...
  function getRegions() {
    return ['ALL'].concat(
      Array.from(
        new Set(
          state.bootstrap.schedules.map(function (item) {
            return item.region;
          }).filter(Boolean)
        )
      )
      ...
      return [
        '<div class="dynamic-slide reveal-on-scroll">',
          '<article class="dynamic-card ' + (idx === 0 ? 'is-dark' : 'is-accent') + '">',
            ...
            points.length ? '<ul class="dynamic-points">' + points.map(function (point) {
    ...
    grid.innerHTML = items.map(function (item) {
      ...
      return [
        '<article class="dynamic-card reveal-on-scroll">',
          ...
          badges.length ? '<div class="cabin-badges">' + badges.map(function (badge) {
            return '<span class="cabin-badge">' + escapeHtml(badge) + '</span>';
          }).join('') + '</div>' : '',
          buildSafeImageHtml_(item.image_url, item.title || ''),
        '</article>'
      ].join('');
  ...
  function buildItineraryTable(days) {
    if (!days.length) {
      return '<div class="schedule-empty" style="margin:18px;">상세 항해일정이 아직 등록되지 않았습니다.</div>';
    }
