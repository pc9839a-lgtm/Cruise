(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      #real-booking-case{padding:92px 0;background:#f4f6f9;color:#10182b}.pmx-case-route{margin:8px auto 0;font-size:clamp(22px,2.5vw,28px);font-weight:600;color:#5b6981;word-break:keep-all}.pmx-case-features{display:grid;grid-template-columns:repeat(3,1fr);max-width:980px;margin:42px auto 0;border-top:1px solid #d9e0ea;border-bottom:1px solid #d9e0ea}.pmx-case-features div{min-height:122px;display:flex;align-items:center;justify-content:center;padding:22px 20px;font-size:clamp(22px,2.45vw,28px);line-height:1.3;font-weight:720;word-break:keep-all}.pmx-case-features div+div{border-left:1px solid #d9e0ea}.pmx-case-prices{display:grid;grid-template-columns:repeat(3,1fr);max-width:980px;margin:48px auto 0}.pmx-case-price{padding:8px 18px}.pmx-case-price span{display:block;font-size:clamp(20px,2.2vw,24px);font-weight:600;color:#68768e}.pmx-case-price strong{display:block;margin-top:12px;font-size:clamp(42px,5.6vw,66px);line-height:.96;font-weight:900;letter-spacing:-.055em;color:#10182b;white-space:nowrap}.pmx-case-price.main strong{color:#245fc4}.pmx-case-note{margin:34px auto 0;font-size:19px;line-height:1.45;font-weight:520;color:#6a778d;word-break:keep-all}
      ...
      @media(max-width:780px){
        ...
        #real-booking-case{padding:72px 0}.pmx-case-route{font-size:21px}.pmx-case-features{grid-template-columns:1fr;margin-top:34px}.pmx-case-features div{min-height:86px;padding:18px 12px;font-size:22px}.pmx-case-features div+div{border-left:0;border-top:1px solid #d9e0ea}.pmx-case-prices{grid-template-columns:1fr;margin-top:34px}.pmx-case-price{padding:18px 10px;border-top:1px solid #d9e0ea}.pmx-case-price:first-child{border-top:0}.pmx-case-price span{font-size:19px}.pmx-case-price strong{font-size:46px}.pmx-case-note{margin-top:24px;font-size:17px}
  ...
  function buildRealCase(after){
    const el=make('section','real-booking-case',`<div class="pmx-inner"><span class="pmx-kicker pmx-enter">실제 예약 예시</span><h2 class="pmx-big-title pmx-enter"><strong>2인 7박 서부 지중해</strong></h2><p class="pmx-case-route pmx-enter">바르셀로나 출발 · MSC World Asia</p><div class="pmx-case-features pmx-enter"><div>2인 기준</div><div>프리미엄 음료 패키지 포함</div><div>디럭스 발코니 Fantastica 객실</div></div><div class="pmx-case-prices pmx-enter"><div class="pmx-case-price"><span>2인 총 예약가</span><strong>$3,887.35</strong></div><div class="pmx-case-price"><span>리워드 포인트 사용</span><strong>1,805.84P</strong></div><div class="pmx-case-price main"><span>총 실지출</span><strong>$2,020.88</strong></div></div><p class="pmx-case-note pmx-enter">예약자·객실번호·가격코드 등 식별정보는 제외했습니다. 예약 조건에 따라 금액은 달라질 수 있습니다.</p></div>`,'pmx-section');
  ...
  function init(){
    ...
    const trust=buildTrust(review),travel=buildTravel(trust),quick=buildQuick(travel),realCase=buildRealCase(quick),bridge=buildBridge(realCase),core=buildCore(bridge),calc=prepareCalculator(core.last),pm=rebuildPriceMatch(calc);
    ...
    watchPlans();
    observe([trust,travel,quick,realCase,bridge,core.start,core.monthly,core.seven,core.payment,core.routes,calc,pm,steps,hotel]);
  }
