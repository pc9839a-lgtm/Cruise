(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      #${SECTION_ID}{position:relative;box-sizing:border-box;width:100%;margin:0;padding:104px 0 112px;overflow:hidden;background:linear-gradient(180deg,#f7f9fc 0%,#eef3f9 100%);color:#0b1730;font-family:Pretendard,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;text-align:center}
      ...
      #${SECTION_ID} .mxt-cards{width:100%;margin:48px auto 0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}
      #${SECTION_ID} .mxt-card{position:relative;box-sizing:border-box;min-height:238px;padding:24px;border:1px solid rgba(22,61,115,.10);border-radius:30px;background:rgba(255,255,255,.90);box-shadow:0 24px 62px rgba(16,40,78,.10);text-align:left;overflow:hidden;opacity:0;transform:translateY(36px) scale(.965);filter:blur(4px);transition:opacity .62s ease,transform .85s cubic-bezier(.16,1,.3,1),filter .62s ease,box-shadow .28s ease}
      ...
      #${SECTION_ID} .mxt-card strong{position:relative;z-index:1;display:block;margin-top:42px;font-size:clamp(29px,2.7vw,38px);line-height:1.08;letter-spacing:-.05em;font-weight:950}
      ...
      #${SECTION_ID} .mxt-hotel-proof{display:none!important}
      ...
      @media(max-width:780px){
        #${SECTION_ID}{padding:72px 0 82px}
        ...
        #${SECTION_ID} .mxt-cards{display:flex;width:100%;gap:12px;overflow-x:auto;scroll-snap-type:x mandatory;overscroll-behavior-x:contain;padding:4px 18px 26px;margin-top:36px;box-sizing:border-box;scrollbar-width:none}
        #${SECTION_ID} .mxt-cards::-webkit-scrollbar{display:none}
        #${SECTION_ID} .mxt-card{flex:0 0 min(82vw,330px);min-height:220px;padding:22px 20px;border-radius:25px;scroll-snap-align:center;box-shadow:0 18px 44px rgba(16,40,78,.11)}
        #${SECTION_ID} .mxt-card::after{right:15px;bottom:-8px;font-size:62px}
        #${SECTION_ID} .mxt-card strong{margin-top:38px;font-size:clamp(29px,8vw,35px)}
