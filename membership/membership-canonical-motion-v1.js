(() => {
  ...
  const cardSelector = [
    ...
    '.mx17-results article','.mx18-card','.mx19-card','.mx22-plans article'
  ].join(',');

  function prepare() {
    const sections = [...document.querySelectorAll(sectionSelectors.join(','))];
    sections.forEach((section) => {
      if (section.dataset.canonMotion === '1') return;
      section.dataset.canonMotion = '1';

      const inner = section.querySelector(':scope > div') || section.querySelector('.mx-inner,.mv2-inner,.m3-inner');
      ...
      [...section.querySelectorAll(cardSelector)].forEach((card, index) => {
        ...
        card.style.setProperty('--canon-delay', `${Math.min(index, 5) * 70}ms`);
      });
    });

    const targets = [...document.querySelectorAll('.canon-reveal:not([data-canon-observed])')];
